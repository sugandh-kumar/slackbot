...
def chat(web_client: slack.WebClient, user_id: str, channel: str, text: str):

    #get the response
    message = "Can You Speak English?"
    if text.lower() == "hi":
...
def message(**payload):
    ...
    """
    data = payload["data"]
    print("data from slack : ", data)
    web_client = payload["web_client"]
    channel_id = data.get("channel")
    user_id = data.get("user")
    text = data.get("text")
    message_type = data.get("subtype")
    if message_type and message_type.lower() == 'bot_message':
        return;
    ...
    if text : 
        if text.lower() == "start":
            ...
            chat(web_client, user_id, channel_id, text)
